...
public class Merge {
     ...
     */
    static native int jniAnalysis(AtomicInteger analysisOut, AtomicInteger preferenceOut, long repoPtr,
            long[] theirHeads);

    /**
     * int git_merge_analysis_for_ref(git_merge_analysis_t *analysis_out,
     * git_merge_preference_t *preference_out, git_repository *repo, git_reference
     * *our_ref, const git_annotated_commit **their_heads, size_t their_heads_len);
     */
    static native int jniAnalysisForRef(AtomicInteger analysisOut, AtomicInteger preferenceOut, long repoPtr,
            long ourRefPtr, long[] theirHeads);;

    /**
     * int git_merge_base(git_oid *out, git_repository *repo, const git_oid *one,
     * const git_oid *two);
     */
    ...
    /**
     * int git_merge_bases(git_oidarray *out, git_repository *repo, const git_oid
     * *one, const git_oid *two);
     */
    static native int jniBases(OidArray outOids, long repoPtr, Oid one, Oid two);

    /**
     * int git_merge_bases_many(git_oidarray *out, git_repository *repo, size_t
     * length, const git_oid [] input_array);
     */
    static native int jniBasesMany(OidArray outOids, long repoPtr, Oid[] inputArray);

    /**
     * int git_merge_commits(git_index **out, git_repository *repo, const git_commit
     * *our_commit, const git_commit *their_commit, const git_merge_options *opts);
     */
    static native int jniCommits(AtomicLong out, long repoPtr, long ourCommit, long theirCommit, long opts);

    /**
     * int git_merge_file( git_merge_file_result *out, const git_merge_file_input
     * *ancestor, const git_merge_file_input *ours, const git_merge_file_input
     * *theirs, const git_merge_file_options *opts);
     */
    static native int jniFile(AtomicLong out, long ancestorPtr, long oursPtr, long theirsPtr, long optsPtr);

    /**
     * int git_merge_file_init_input(git_merge_file_input *opts, unsigned int
     * version);
     */
    static native int jniFileInitInput(long opts, int version);

    /**
     * int git_merge_file_init_options(git_merge_file_options *opts, unsigned int
     * version);
     */
    ...
    /**
     * void git_merge_file_result_free(git_merge_file_result *result); Note: this
     * also frees the resultPtr itself.
     */
    ...
    /**
     * int git_merge_create( git_repository *repo, const git_annotated_commit
     * **their_heads, size_t their_heads_len, const git_merge_options *merge_opts,
     * const git_checkout_options *checkout_opts);
     */
    static native int jniMerge(long repoPtr, long[] theirHeads, long mergeOptsPtr, long checkoutOpts);
    ...
    /**
     * int git_merge_trees( git_index **out, git_repository *repo, const git_tree
     * *ancestor_tree, const git_tree *our_tree, const git_tree *their_tree, const
     * git_merge_options *opts);
     */
    static native int jniTrees(AtomicLong out, long repoPtr, long ancestorTree, long ourTree, long theirTree,
            long opts);
     ...
     * @throws GitException git errors 0 on success, GIT_ENOTFOUND if not found or
     *                      error code
     */
    @Nonnull
     ...
     * out. If the index is to be converted to a tree, the caller should resolve any
     ...
     */
    @Nonnull
    public static Index trees(@Nonnull Repository repo, @Nullable Tree ancestorTree, @Nonnull Tree ourTree,
            @Nonnull Tree theirTree, @Nullable Options opts) {
        Index out = new Index(0);
        Error.throwIfNeeded(
                jniTrees(out._rawPtr, repo.getRawPointer(), ancestorTree == null ? 0 : ancestorTree.getRawPointer(),
                        ourTree.getRawPointer(), theirTree.getRawPointer(), opts == null ? 0 : opts.getRawPointer()));
        return out;
    }
     ...
     */
    @Nonnull
    public static Index commits(@Nonnull Repository repo, @Nullable Commit ourCommit, @Nullable Commit theirCommit,
            @Nullable Options opts) {
        Index out = new Index(0);
        Error.throwIfNeeded(jniCommits(out._rawPtr, repo.getRawPointer(),
                ourCommit == null ? 0 : ourCommit.getRawPointer(),
                theirCommit == null ? 0 : theirCommit.getRawPointer(), opts == null ? 0 : opts.getRawPointer()));
        return out;
    }
     ...
     */
    @Nonnull
    public static AnalysisPair analysis(@Nonnull Repository repo, @Nonnull List<AnnotatedCommit> theirHeads) {
        AtomicInteger outAnalysis = new AtomicInteger();
        AtomicInteger outPreference = new AtomicInteger();
        Error.throwIfNeeded(jniAnalysis(outAnalysis, outPreference, repo.getRawPointer(),
                theirHeads.stream().mapToLong(AnnotatedCommit::getRawPointer).toArray()));
        return new AnalysisPair(IBitEnum.valueOf(outAnalysis.get(), AnalysisT.class),
     ...
     */
    @Nonnull
    public static AnalysisPair analysisForRef(@Nonnull Repository repo, @Nullable Reference ourRef,
            @Nonnull List<AnnotatedCommit> theirHeads) {
        AtomicInteger outAnalysis = new AtomicInteger();
        AtomicInteger outPreference = new AtomicInteger();
        Error.throwIfNeeded(jniAnalysisForRef(outAnalysis, outPreference, repo.getRawPointer(),
     ...
     * @param repo       the repository where the commits exist
     * @param inputArray oids of the commits
     * @return the OID of a merge base considering all the commits or empty if not
     *         found
     * @throws GitException git errors
     */
    @Nonnull
     ...
     *
     * <p>
     * Note that this function does not reference a repository and any configuration
     * must be passed as `git_merge_file_options`.
     *
     ...
     */
    @Nonnull
    public static FileResult file(@Nonnull FileInput ancestor, @Nonnull FileInput ours, @Nonnull FileInput theirs,
            @Nullable FileOptions opts) {
        FileResult result = new FileResult(false, 0);
        Error.throwIfNeeded(jniFile(result._rawPtr, ancestor.getRawPointer(), ours.getRawPointer(),
                theirs.getRawPointer(), CAutoReleasable.rawPtr(opts)));
        return result;
    }

    /**
     * Merges the given commit(s) into HEAD, writing the results into the working
     * directory. Any changes are staged for commit and any conflicts are written to
     * the index. Callers should inspect the repository's index after this
     * completes, resolve any conflicts and prepare a commit.
     *
     * <p>
     * For compatibility with git, the repository is put into a merging state. Once
     * the commit is done (or if the uses wishes to abort), you should clear this
     * state by calling `git_repository_state_cleanup()`.
     *
     * @param repo         the repository to merge
     * @param theirHeads   the heads to merge into
     ...
     * @throws GitException git errors
     */
    public static void merge(@Nonnull Repository repo, @Nonnull List<AnnotatedCommit> theirHeads,
            @Nullable Options mergeOpts, @Nullable Checkout.Options checkoutOpts) {
        Error.throwIfNeeded(
                jniMerge(repo.getRawPointer(), theirHeads.stream().mapToLong(CAutoReleasable::getRawPointer).toArray(),
    ...
    public enum FlagT implements IBitEnum {
        /**
         * Detect renames that occur between the common ancestor and the "ours" side or
         * the common ancestor and the "theirs" side. This will enable the ability to
         * merge between a modified and renamed file.
         */
        FIND_RENAMES(1 << 0),

        /**
         * If a conflict occurs, exit immediately instead of attempting to continue
         * resolving conflicts. The merge operation will fail with GIT_EMERGECONFLICT
         ...
         * recursive merge base (by merging the multiple merge bases), instead simply
         * use the first base. This flag provides a similar merge base to
         * `git-merge-resolve`.
         */
        NO_RECURSIVE(1 << 3);
    ...
    public enum FileFavorT {
        ...
        /**
         * When a region of a file is changed in both branches, the file created in the
         * index will contain the "ours" side of any conflicting region. The index will
         * not record a conflict.
         */
        OURS(1),

        /**
         * When a region of a file is changed in both branches, the file created in the
         * index will contain the "theirs" side of any conflicting region. The index
         * will not record a conflict.
         */
        THEIRS(2),

        /**
         * When a region of a file is changed in both branches, the file created in the
         * index will contain each unique line from each side) which has the result of
         * combining both files. The index will not record a conflict.
         */
        UNION(3);
    ...
    public enum AnalysisT implements IBitEnum {
         ...
         * performed. Instead, the client can check out the given merge input.
         */
        FASTFORWARD(1 << 2),

        /**
         * The HEAD of the current repository is "unborn" and does not point to a valid
         * commit. No merge can be performed, but the caller may wish to simply set HEAD
         * to the target commit(s).
         */
        UNBORN(1 << 3);
     ...
     * `git_merge_file_input` structure with descriptions of the files in each side
     * of the conflict for use in producing the merge file.
     */
    ...
    /**
     * POJO contains analysis result from {@code Merge::analysis} and
     * {@code Merge::analysisForRef}
     */
