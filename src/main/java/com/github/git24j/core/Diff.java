...
public class Diff extends CAutoReleasable {
     ...
     * @throws GitException git errors
     */
    public static Diff treeToTree(Repository repo, Tree oldTree, Tree newTree, DiffOptions opts) {
     ...
     * @throws GitException git errors
     */
    public static Diff treeToIndex(Repository repo, Tree oldTree, Index index, DiffOptions opts) {
        Diff diff = new Diff(false, 0);
        int e =
                jniTreeToIndex(
                        diff._rawPtr,
     ...
     * @throws GitException git errors
     */
    public static Diff indexToWorkdir(Repository repo, Index index, DiffOptions opts) {
        Diff diff = new Diff(false, 0);
        Error.throwIfNeeded(
                jniIndexToWorkdir(
                        diff._rawPtr,
     ...
     * @throws GitException git errors
     */
    public static Diff treeToWorkdir(Repository repo, Tree oldTree, DiffOptions opts) {
        Diff diff = new Diff(false, 0);
        Error.throwIfNeeded(
                jniTreeToWorkdir(
                        diff._rawPtr,
     ...
     * @throws GitException git errors
     */
    public static Diff treeToWorkdirWithIndex(Repository repo, Tree oldTree, DiffOptions opts) {
        Diff diff = new Diff(false, 0);
        Error.throwIfNeeded(
    ...
    public static int blobs(
        ...
        long jniOptions =
                options == null
                        ? DiffOptions.create(DiffOptions.CURRENT_VERSION).getRawPointer()
